<template>
	<van-row>
		<van-col span="24">
			<van-search placeholder="请输入搜索关键词" v-model="query.keyWord"/>
		</van-col>
		<transition>
		...
		<van-col span="24">
			<van-pull-refresh :value="index.loading" @refresh="refreshList">
				<van-list
					...
					@load="refreshList">
					<van-cell v-for="item in index.list" :key="item.id" :title="item.name" :value="item.typeName" :label="item.grade"/>
				</van-list>
			</van-pull-refresh>
		</van-col>
	</van-row>
...
export default{
	...
	,created(){
		this.query = {...this.query,...this.index.query};
		this.loadMore();
	}
	,methods:{
		refreshList(){
